...
import com.tomaytotomato.location4j.model.search.ScoredResult;
...
public class SearchLocationService implements SearchLocation, SearchCountry, SearchState,
	SearchCity {
  ...
  protected SearchLocationService(TextTokeniser textTokeniser, TextNormaliser textNormaliser,
	...
	this.countryNativeNameToCountryMap = new HashMap<>(
		location4JData.getCountryNativeNameToCountryMap());
  ...
  @Override
  public List<SearchLocationResult> search(String text,
	  Class<? extends SearchLocationResult> resultType) {
	return List.of();
  }
   ...
   * @return a list of search location results for direct matches Note: results size will be 0 or 1
   * for countries, states, and cities respectively
   */
  private List<SearchLocationResult> findDirectMatches(String text) {
	...
	if (cityNameToCitiesMap.containsKey(text)) {
	  cityNameToCitiesMap.get(text)
		  .forEach(city -> matches.add(searchLocationResultMapper.toCityResult(city)));
	  return matches;
	}
  ...
  /**
   * Finds all matches for countries, states, and cities from the tokenized text. Then builds and
   * prioritizes search results based on hierarchical scoring.
   *
  ...
  /**
   * Builds and prioritizes search results based on hierarchical scoring. All countries are given a
   * baseline score as they are at the top of the hierarchy. States are scored higher if their
   * parent country is also matched. Cities are scored highest if both their parent state and
   * country are matched.
   *
  ...
  private int calculateStateScore(StateResult state, List<CountryResult> countryMatches) {
	...
	var stateCountryMatched = countryMatches.stream()
  ...
  private List<CityResult> findAllCityMatches(List<String> tokenizedText) {
	...
	for (String token : tokenizedText) {
	  if (cityNameToCitiesMap.containsKey(token)) {
		cityNameToCitiesMap.get(token)
			.forEach(city -> cityMatches.add(searchLocationResultMapper.toCityResult(city)));
	  }
	}
	return cityMatches;
  }
  ...
  private List<StateResult> findAllStateMatches(List<String> tokenizedText) {
	...
	for (String token : tokenizedText) {
	  stateMatches.addAll(searchStates(token));
	}
	return stateMatches;
  }
  ...
  @Override
  public List<CountryResult> searchCountries(String searchText) {
	var countryMatches = new ArrayList<CountryResult>();
	if (countryNameToCountryMap.containsKey(searchText)) {
	  var country = countryNameToCountryMap.get(searchText);
	  countryMatches.add(searchLocationResultMapper.toCountryResult(country));
	} else if (countryNativeNameToCountryMap.containsKey(searchText)) {
	  var country = countryNativeNameToCountryMap.get(searchText);
	  countryMatches.add(searchLocationResultMapper.toCountryResult(country));
	} else if (iso2CodeToCountryMap.containsKey(searchText)) {
	  var country = iso2CodeToCountryMap.get(searchText);
	  countryMatches.add(searchLocationResultMapper.toCountryResult(country));
	} else if (iso3CodeToCountryMap.containsKey(searchText)) {
	  var country = iso3CodeToCountryMap.get(searchText);
	  countryMatches.add(searchLocationResultMapper.toCountryResult(country));
	}
	return countryMatches;
  }
  ...
  @Override
  public List<CityResult> searchCities(String searchText) {
	var cityMatches = new ArrayList<CityResult>();
	if (cityNameToCitiesMap.containsKey(searchText)) {
	  cityNameToCitiesMap.get(searchText)
		  .forEach(city -> cityMatches.add(searchLocationResultMapper.toCityResult(city)));
	}
	return cityMatches;
  }
  ...
  @Override
  public List<StateResult> searchStates(String searchText) {
	var stateMatches = new ArrayList<StateResult>();
	if (stateNameToStatesMap.containsKey(searchText)) {
	  stateNameToStatesMap.get(searchText)
		  .forEach(state -> stateMatches.add(searchLocationResultMapper.toStateResult(state)));
	} else if (stateNativeNameToStateMap.containsKey(searchText)) {
	  stateNativeNameToStateMap.get(searchText)
		  .forEach(state -> stateMatches.add(searchLocationResultMapper.toStateResult(state)));
	} else if (stateCodeToStatesMap.containsKey(searchText)) {
	  stateCodeToStatesMap.get(searchText)
		  .forEach(state -> stateMatches.add(searchLocationResultMapper.toStateResult(state)));
	}
	return stateMatches;
  }
